...
def campo_valore(nome, default=""):
    if not df.empty and nome in df.columns:
        return st.text_input(nome, value=str(df[nome].iloc[-1]))
...
diametri_monte = st.number_input("Diametri Idraulici a Monte", step=0.1)
diametri_valle = st.number_input("Diametri Idraulici a Valle", step=0.1)
...
for i in range(1, num_prelievi + 1):
    ...
    ugello = st.text_input(f"Ugello prelievo {i}", key=f"ugello_{i}")
    ...
    if calcola_volume:
        ...
        # Selezione fra i precedenti o input manuale
        if volume_normalizzati_esistenti:
            volume_normalizzato = st.selectbox(f"Scegli Volume Normalizzato {i} dai prelievi precedenti", volume_normalizzati_esistenti, key=f"vol_norm_sel_{i}")
        else:
    ...
    nuovi_prelievi.append({
        ...
        "Certificato Bombola Mix": cert_bombola_mix,
        "Certificato Bombola O2": cert_bombola_o2,
        "PC": pc,
        "Laser": laser,
        ...
        "PesoInizialeSerpentina": peso_in_serp,
        "PesoFinaleSerpentina": peso_fin_serp,
        "PesoInizialeGel": peso_in_gel,
        "PesoFinaleGel": peso_fin_gel,
        "UmiditàFumi": umidita_fumi,
        "Isocinetismo": isocinetismo,
        "Velocità": velocita_media,
        "dP": dp,
        "TemperaturaFumi": temp_fumi,
        "Note": note
    })

# --- Pulsante Salva su Google Sheets ---
